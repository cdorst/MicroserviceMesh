...
namespace Declaration.Generator.Internals.CodeTemplates
{
    internal class DeclarationLayer : DeclarationType
    {
        ...
        private static readonly UsingDirectiveList UsingDirectives = Usings.Create("System.Collections.Generic");
        ...
        public static DeclarationFile GetLayer(in Layer layer, in string path, in IConfigurationRoot configuration)
            => new DeclarationFile(
                GetTypeDeclaration(Layer, GetTypeNamespace(in path), in configuration, in XmlDocSummary, in UsingDirectives),
                path, Layer);
    }
}
